...
void Roche::roche_shadow(double q, double iangle, double phi, double dist, double acc, float x[], float y[], bool shade[], int n){
  ...
  Subs::Vec3 cofm(1,0,0), earth(cos(2*M_PI*phi)*sin(2*M_PI*iangle/360),-sin(2*M_PI*phi)*sin(2*M_PI*iangle/360),cos(2*M_PI*iangle/360));
  Subs::Vec3 p, dirn, p1, p2;
  p.y()   = p.z() = 0.;
  p.x()   = rl1;
  ...
  for(int i=0; i<n; i++){
    ...
    if(!blink(q, p1, earth, acc)){
      x[i]     = p1.x();
      y[i]     = p1.y();
      shade[i] = false;
    }else if(blink(q, p2, earth, acc)){
      x[i]     = p2.x();
      y[i]     = p2.y();
      shade[i] = true;
    }else{
