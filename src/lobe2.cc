 ...
 */
void Roche::lobe2(double q, float x[], float y[], int n){
  ...
  if(q <= 0.)
    throw Roche_Error("Roche::lobe2(double, float*, float*, int): q = " + Subs::str(q) + " <= 0.");

  if(n < 3) 
    throw Roche_Error("Roche::lobe2(double, float*, float*, int): n = " + Subs::str(n) + " < 3.");
  ...
  for(int i=0;i<n;i++){
    ...
    if(i == 0 || i == n-1){
      ...
      rad   = Subs::rtsafe(Lfunc2(dx,dy,q,cpot), lower, upper, FRAC);
